def main():
    if __name__ == "__main__": 
        ...
        import numpy as np
        from funk import download1
        from funk import download2
        from funk import download3
        from funk import download4
        import multi4
        ...
        try:
            ...
            #now = UTCDateTime("2021-02-18T12:15:00")
                ...
                for i in files:
                    if list(i.split("."))[-1] == "mseed":
                        ...
                        try:
                            ...
                            pass
            folder = 'C:\\Users\\Lovro Milišić\\Desktop\\Seizmografi\\pics'
        ...
        """)
                ...
                f.close()
                ###print(eq[i])
                if (len(eq)>=1 and round(mags[i], 2)>=2.50 and not bio):
                    try:
                        response = client.chat_postMessage(channel="U01JCRMUM7U", text="================================")
                    except:
                        pass
                if round(mags[i], 2)>=2.50:
                    try:
                        ime = "map"+a+".html"
            ...
            print(e)
        try:
            response = client.chat_postMessage(channel="U01JCRMUM7U", text="--- %s seconds ---" % (time.time() - start_time))
        except Exception as e:
            print(e)
        #print("--- %s seconds ---" % (time.time() - start_time))
        from subprocess import Popen
        p = Popen("run.bat", cwd=r"C:\Users\Lovro Milišić\Desktop\Seizmografi")
        stdout, stderr = p.communicate()
        import os
        clear = lambda: os.system('taskkill /PID '+str(os.getppid())+' /T /F')
        clear()
